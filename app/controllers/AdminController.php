...
class AdminController extends ControllerBase
{
  ...
  public function initialize() {
    ...
    if('login' != $this->getActionName()) {
      ...
      //检查是否登录
      //检查时限
      if(!$name ||!$validTime||!$crypt||$validTime < time()||$crypt!= $this->encodeCrypt($name, $validTime)){
      ...
      //读取服务器列表
      if(!$ajax){
        $ServerList = new LoginServerList;
        $serverList = $ServerList->dicGetAll();
        $currentServer = '';
        $currentServerId = 0;
        foreach($serverList as $_l){
          if($_l['game_server_host'] == 'http://'.$_SERVER['HTTP_HOST']){
  ...
  public function dashboardAction(){
    ...
    if(!$this->user['pwd_status'] && !@$_COOKIE['sg2admin_pleaseChangePwd']){
      $this->view->setVar("pleaseChangePwd", 1);
      $timeout = time()+10*3600;
      setcookie('sg2admin_pleaseChangePwd', 1, $timeout, '/');
    }
    //总玩家数
    $this->view->setVar("playerAllCounter",Player::count());
    //今日新增玩家
    $this->view->setVar("playerNewCounter",Player::count(['create_time >="'.date('Y-m-d 00:00:00').'"']));
    //今日活跃用户数
    $this->view->setVar("playerLoginCounter",Player::count(['login_time >="'.date('Y-m-d 00:00:00').'"']));
    
    //计算在线人数-大概 
    $data = ['Msg'=>'DataRequest', 'Type'=>'all_conn_info', 'Data'=>[]];
    $re = socketSend($data);
    $r = json_decode($re['content'], true);
    $this->view->setVar("playerOnlineCounter",count($r));
  ...
  public function encodeCrypt($name, $timeout){
    return md5($name . $timeout . LoginAdminUser::AUTH_SECRET);
  }
  ...
  public function checkAuthId($authId, $failop=1){
    ...
    if($user['auth']['auth'] !== true){
      if(!in_array($authId, $user['auth']['auth'])){
        ...
        $this->view->setVar("isRoot", false);
      }
    }
    else{
      $this->view->setVar("isRoot", true);
    }
    return true;
  }
  ...
  public function ajaxMultiServerHandleAction(){
    ...
    foreach($serverList as $_server){
      ...
      $_ret = curlPost($_server['game_server_host'].'/admin/'.$action, $fields);
  ...
  public function adminManagerAction(){
    ...
    //获取所有管理员
    $admins = (new LoginAdminUser)->find(['status=0'])->toArray();
    $auths = (new LoginAdminAuth)->find()->toArray();
    $_auths = [];
    foreach($auths as $_auth){
      $_auths[$_auth['id']] = $_auth;
    }
    $this->view->setVars(["admins"=>$admins, "auths"=>$_auths]);
  }
  ...
  public function adminAddAction(){
    ...
    if(!(new LoginAdminAuth)->find($adminAuth)){
      echo json_encode(['err'=>'权限不存在']);
      exit;
    }
    if(!(new LoginAdminUser)->add($adminName, $adminAuth)){
  ...
  public function adminDeleteAction(){
    ...
    if(!(new LoginAdminUser)->find($adminId)->delete()){
  ...
  public function adminModifyAuthAction(){
    ...
    if(!(new LoginAdminAuth)->find($auth)){
      echo json_encode(['err'=>'权限不存在']);
      exit;
    }
    if(!(new LoginAdminUser)->updateAll(['auth_type'=>$auth], ['id'=>$adminId])){
  ...
  public function adminAuthManagerAction(){
    ...
    // var_dump($_auths);
    // exit;
    $this->view->setVars(["auths"=>$_auths, "authType"=>$this->authType]);
  }
  
  //[权限编辑]:新增一项权限拥有者(包含其所拥有哪些权限)
  public function adminAuthAddAction(){
    ...
    if((new LoginAdminAuth)->find(['name="'.$authName.'"'])->toArray()){
      echo json_encode(['err'=>'该名字已经存在']);
      exit;
    }
    if(!(new LoginAdminAuth)->add($authName, $authInfo)){
  ...
  public function adminAuthDeleteAction(){
    ...
    if(!(new LoginAdminAuth)->find($authId)->delete()){
      echo json_encode(['err'=>'删除失败']);
      exit;
    }
    $memo = [
  ...
  public function modifyAuthAction(){
    ...
    $adminAuth = (new LoginAdminAuth)->findFirst($authId);
    if(!$adminAuth) {
      exit;
    }
    ...
    if(!(new LoginAdminAuth)->updateAll(['name'=>"'".$name."'", 'auth'=>"'".join(',', $auth)."'"], ['id'=>$authId])){
      echo json_encode(['err'=>'修改权限失败']);
      exit;
    }
    $memo = [
  ...
  public function playerInfoAction() {
    ...
    $this->view->setVar("playerInfo", $playerInfo);
    
    $this->view->banTime = (new PlayerInfo)->getBanTime($playerId);
    
    // $this->view->disableLowerCase();  

    //获取平台用户名
    $channel = $playerInfo['login_channel'];
    $this->view->setVar("registname",'[渠道:'.$channel.']');
    // try {
  ...
  public function banAccountAction(){
    ...
    $banTime = '';
    if($playerInfo['ban_time'] && $playerInfo['ban_time'] >= time()) {
  ...
  public function doNotBanAccountAction(){
    ...
    $PlayerInfo = new PlayerInfo;
    $PlayerInfo->alter($playerId, ['ban_time'=>'1970-01-01 00:00:00']);
    exit;
  }
  ...
  //[操作玩家]:发送道具
  public function playerSendItemAction(){
    $this->view->setVar("treeact",'playerdo');
  ...
  public function ajaxPlayerSendItemAction(){

    $authCode = 203;
    if(!$this->checkAuthId($authCode, 2)) {
      exit;
    }
    $type = $_REQUEST['playerToType']; //type:1--全体玩家, 2：指定玩家
    $failPlayer = [];
    if($type == 2){
  ...
  public function ajaxClearServerCacheAction(){
    ...
    foreach($server as $k=>$v) {
      $nodes[$k]['url']    = $serverList[$v]['game_server_host'] . '/api/clearCache';
  ...
  public function ajaxAlterServerListFieldAction(){
    ...
    $field = $_POST['field'];
    $value = $_POST['value'];
    if(in_array($field, ['status', 'is_new', 'default_enter', 'maintain_notice', 'all_status'])) {
      if($field=='default_enter') {
        (new LoginServerList)->alterDefaultEnter($id);
      } 
      elseif($field=='all_status') {
        (new LoginServerList)->alterAllStatus($value);
      } 
      else {
        (new LoginServerList)->alterServerList($id, $field, $value);
      }
    }
    $memo = [
      'desc'  => '修改' . $field,
    ...
    foreach($cacheKeys as $_k){
      ...
      if(count($_k) > 1){
        ...
        if(isset($cacheArr[$_k[1]])){
          Cache::dbByName($_k[1], $otherList[$_k[0]])->flushDB();
        }
      }else{
        $cacheArr = $config->redis->index->toArray();
        if(isset($cacheArr[$_k[0]])){
          Cache::dbByName($_k[0])->flushDB();
        }
      }
    }
