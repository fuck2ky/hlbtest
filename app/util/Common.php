...
/**
 * 输出log 到debug.log文件中,需要constant中开启 DEBUG_LOG_ON 为true
 * 
 * 使用方法如下
 * ```php
 *       debug('333');
 ...
 */
function phplog($o){
    file_put_contents(APP_PATH."/logs/debug.log", $o, FILE_APPEND);
}
...
/**
 * 加密给前端
 * 注意 aesEncode 和 aesDecode 中的 openssl_encrypt 和 openssl_decrypt 用到的参数标志不一致！！fuck !
 */
function aesEncode($content){
    ...
    //$encrypted = mcrypt_encrypt(MCRYPT_RIJNDAEL_128, $key, $content, MCRYPT_MODE_CBC, $iv); //php5
    $encrypted = openssl_encrypt($content, "aes-256-cbc", $key, OPENSSL_RAW_DATA, $iv); //php7
    $re        = pack('N1', $length).$encrypted; //前面4个字节插入长度, 大端模式
    return $re;
}
...
function aesDecode($binaryData){
    $data      = unpack("N1length", $binaryData); //按大端模式求得数据长度(4个字节),并保存到下标变量$length
    $content   = substr($binaryData, 4, strlen($binaryData) - 4);
    $length    = $data['length'];
    $key       = pack('A*', 'hdkjfhxcbvuiuri3289492hdskjfhadr');//key定义为：char aes_key_buf[32] = "ijshanckdwiiuedhsiqo0955;[jsmc4";
    $iv        = pack("c16", 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00);//iv定义为 16字节的\0字符
    //$decrypted = mcrypt_decrypt(MCRYPT_RIJNDAEL_128, $key, $content, MCRYPT_MODE_CBC, $iv); //php5
    $decrypted = openssl_decrypt($content, "aes-256-cbc", $key, OPENSSL_RAW_DATA|OPENSSL_ZERO_PADDING, $iv); //php7 
...
function decodePostData($postData){
    ...
    if(!validateStr($originStr, $hashStr)) {//校验参数 md5 hash
...
function getPost(){
    ...
    if(isset($_GET['inner_flag'])) {//是否内部访问
...
function comboRequest(array $nodes, $postData, $uuid){
    global $config;
    $innerUrl  = (new LoginServerList)->getGameServerIpByServerId($config->server_id);//'http://32.cn';
    $response  = ['code'=>0, 'data'=>[], 'basic'=>[], 'steps'=>[], 'exec_time'=>[]];
    
    foreach($nodes as $node) {
        $fields = $postData;
        $fields['inner'] = 1;
        if(isset($node['para'])) { //参数
            $fields = array_merge($fields, $node['para']);
        }
        $url = strtolower($node['url']); //访问的方法(method)        
        $_url = $innerUrl . '/' . $url . '?inner_flag=1';
...
function dbBegin($db){
    $db->begin();
    // CollectionBase::begin(); //Collection 新版本接口有改动,暂时屏蔽掉
...
// swoole task log, 保存在 app/logs/swoole.log 
